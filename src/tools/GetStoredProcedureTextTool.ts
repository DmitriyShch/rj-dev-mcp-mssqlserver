...
type GetStoredProcedureTextParams = {
...
type GetStoredProcedureTextResult = {
  success: true;
  storedProcName: string;
  schemaName?: string;
  startLine: number;
...
type GetStoredProcedureTextError = {
  success: false;
  message: string;
};

// Small safety buffer to keep responses well below the ~10KB transport threshold we observed.
...
export class GetStoredProcedureTextTool implements Tool {
  ...
  description = "Gets the text of a specified MSSQL Database stored procedure. Supports optional schema and line slicing.";
  inputSchema = {
    type: "object",
    properties: {
  ...
  async run(params: GetStoredProcedureTextParams): Promise<GetStoredProcedureTextResult | GetStoredProcedureTextError> {
    try {
      const { storedProcName, schemaName, startLine, lineCount, startChar } = params;
      ...
      if (!record?.definition) {
        return {
          success: false,
          message: schemaName
            ? `Stored procedure ${schemaName}.${storedProcName} was not found.`
            : `Stored procedure ${storedProcName} was not found.`,
        };
      }
      ...
      // Determine effective start line (1-based) corresponding to the chosen character index.
      ...
      if (hasMore) {
        ...
        while (
          lineIndexForEnd + 1 < lineStartIndices.length &&
          lineStartIndices[lineIndexForEnd + 1] <= endCharExclusive
        ) {
          lineIndexForEnd += 1;
        }
